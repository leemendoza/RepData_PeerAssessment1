...
#####The mean number of steps per day is `r sprintf("%.2f", mean(steps_each_day$Total, na.rm=TRUE))`, and the median number of steps per day is `r median(steps_each_day$Total, na.rm=TRUE)`
***
...
```
#####The interval with the maximum number of steps across all days is interval `r max_interval`
***
...
```
#####There are `r miss` missing values in the original data set  
